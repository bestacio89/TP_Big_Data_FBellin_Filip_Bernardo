...
def nettoyer_fichier(fichier_entree, fichier_sortie):
    ...
    # Gérer les valeurs manquantes (à adapter selon les besoins de chaque tâche)
    # Exemple : remplacer les valeurs manquantes de la colonne `timbrecli` par la moyenne
    df['timbrecli'].fillna(df['timbrecli'].mean(), inplace=True)

    # ... (autres traitements pour les valeurs manquantes)
...
nettoyer_fichier("./dataw_fro03.csv", "./dataw_fro03_cleaned.csv")  
nettoyer_fichier("./dataw_fro03_mini_1000.csv", "./dataw_fro03_mini_1000_cleaned.csv")